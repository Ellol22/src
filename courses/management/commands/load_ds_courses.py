...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        json_path = 'data/ds_courses.json'  # عدل المسار لو الملف في مكان تاني
        try:
        ...
        deleted, _ = Course.objects.filter(structure__department=DepartmentChoices.DATA).delete()
        ...
        for year_key, year_data in ds_courses.items():
            ...
            for term_key, courses in year_data.items():
                ...
                try:
                    ...
                    self.stdout.write(self.style.WARNING(
                        f"⚠️ No StudentStructure found for Data Science, {academic_year}, {semester}"
                    ))
                    continue
